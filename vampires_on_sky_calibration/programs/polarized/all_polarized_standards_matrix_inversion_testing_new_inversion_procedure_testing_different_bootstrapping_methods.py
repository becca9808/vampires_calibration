...
# # Bootstrapping Errorbars - Resampling Matrix Inverted Values
...
# Function to calculate pol_percent and aolp errors using bootstrapping
...
for obj in unique_objects:
    ...
    for (altitude, pol_percent) in obj_pol_list:
        ...
        # Ensure the variables are scalars before formatting
    ...
    for (altitude, aolp) in obj_aolp_list:
        whittet_aolp = targets[obj]["675nm"][2]
        aolp_residual = (aolp % 180) - whittet_aolp
...
# # Resampling Residuals - Re-Attempt to Follow Unpolarized Standards 
...
# # Bootstrapping Errors - Resampling the residuals
...
pol_err_list = []
aolp_err_list = []
final_altitudes_dict = {obj: [] for obj in unique_objects}
...
def bootstrap_residuals(Q_data, U_data, Q_fitted, U_fitted, num_bootstrap_samples):
    Q_residuals = Q_data - Q_fitted
    U_residuals = U_data - U_fitted
    
    Q_bootstrap = np.zeros((num_bootstrap_samples, len(Q_data)))
    U_bootstrap = np.zeros((num_bootstrap_samples, len(U_data)))
    ...
    return Q_err_bootstrap, U_err_bootstrap
...
for obj in unique_objects:
    ...
    for i, row in object_data.iterrows():
        ...
        if data_Q != 0:
            ...
            # Append Q and U data to the lists
            Q_data.append(Q)
            U_data.append(U)
            ...
            # Saving the mean of the altitudes
            mean_altitude = np.mean(this_cycle_altitudes)
            final_altitudes_dict[obj].append(mean_altitude)
    ...
    # Convert Q and U data to numpy arrays
    Q_data = np.array(Q_data)
    U_data = np.array(U_data)
    
    # Fit model to Q and U data (assume model fitting step is defined elsewhere)
    # For demonstration, we'll use Q_fitted and U_fitted as mean values
    Q_fitted = np.mean(Q_data) * np.ones_like(Q_data)
    U_fitted = np.mean(U_data) * np.ones_like(U_data)
    
    # Bootstrap residuals to estimate errors
    Q_err_bootstrap, U_err_bootstrap = bootstrap_residuals(Q_data, U_data, Q_fitted, U_fitted, num_bootstrap_samples)

    # Append results to lists
    for altitude, Q, U, Q_err, U_err in zip(final_altitudes_dict[obj], Q_data, U_data, Q_err_bootstrap, U_err_bootstrap):
        Q_list.append((altitude, Q, color, obj))
        U_list.append((altitude, U, color, obj))
        Q_err_list.append(Q_err)
        U_err_list.append(U_err)

        # Calculate degree and angle of linear polarization with errors
        pol_percent, aolp = funcs.stokes_to_deg_pol_and_aolp(Q, U)
        pol_percent_err, aolp_err = funcs.stokes_to_deg_pol_and_aolp_errors(Q, U, Q_err, U_err)
...
for (altitude, pol_percent, color, obj), pol_err in zip(pol_list, pol_err_list):
    label = f'{obj} (Data)' if f'{obj} (Data)' not in plotted_labels else None
    ax1.errorbar(altitude, pol_percent, yerr=pol_err, label=label, linestyle='None', marker='o', color=color, markerfacecolor=color)
    print(f"Altitude: {altitude} | Object: {obj} | DoLP: {pol_percent} | Error: {pol_err}")
    plotted_labels.add(f'{obj} (Data)')
